#include "sm4_ttable.h"
#include <iostream>
#include <iomanip>
#include <vector>
#include <numeric>
#include <random>
#include <chrono>
#include <stdexcept>

#define rotl(x, n) (((x) << (n)) | ((x) >> (32 - (n))))

word32 SM4_TTable_Context::T_tables[4][256];

static const word8 Sbox[256] = {
    
};

static const word32 FK[4] = {0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};
static const word32 CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

inline word32 get_word(const word8* p) {
    return ((word32)p[0] << 24) | ((word32)p[1] << 16) | ((word32)p[2] << 8) | (word32)p[3];
}

inline void put_word(word32 w, word8* p) {
    p[0] = (word8)(w >> 24);
    p[1] = (word8)(w >> 16);
    p[2] = (word8)(w >> 8);
    p[3] = (word8)w;
}

inline word32 T_lookup(word32 x) {
    return SM4_TTable_Context::T_tables[0][(x >> 24) & 0xFF] ^
           SM4_TTable_Context::T_tables[1][(x >> 16) & 0xFF] ^
           SM4_TTable_Context::T_tables[2][(x >> 8) & 0xFF] ^
           SM4_TTable_Context::T_tables[3][x & 0xFF];
}

void SM4_TTable_Context::generate_ttables() {
    for (int i = 0; i < 256; ++i) {
        word32 s = Sbox[i];
        word32 t = s ^ rotl(s, 2) ^ rotl(s, 10) ^ rotl(s, 18) ^ rotl(s, 24);
        T_tables[0][i] = rotl(t, 24);
        T_tables[1][i] = rotl(t, 16);
        T_tables[2][i] = rotl(t, 8);
        T_tables[3][i] = t;
    }
}

SM4_TTable_Context::SM4_TTable_Context() {
    static bool tables_generated = false;
    if (!tables_generated) {
        generate_ttables();
        tables_generated = true;
    }
}

static word32 T_prime_transform(word32 x) {
    word32 result = 0;
    result |= ((word32)Sbox[(word8)(x)]) << 0;
    result |= ((word32)Sbox[(word8)(x >> 8)]) << 8;
    result |= ((word32)Sbox[(word8)(x >> 16)]) << 16;
    result |= ((word32)Sbox[(word8)(x >> 24)]) << 24;
    return result ^ rotl(result, 13) ^ rotl(result, 23);
}

void SM4_TTable_Context::set_key(const word8 key[16]) {
    word32 MK[4];
    for (int i = 0; i < 4; ++i) {
        MK[i] = get_word(key + i * 4);
    }

    word32 K[36];
    for (int i = 0; i < 4; ++i) {
        K[i] = MK[i] ^ FK[i];
    }

    for (int i = 0; i < 32; ++i) {
        K[i + 4] = K[i] ^ T_prime_transform(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i]);
        this->round_keys[i] = K[i + 4];
    }
}

void SM4_TTable_Context::process_block(const word8 input[16], word8 output[16], bool is_encrypt) {
    word32 X[4];
    for (int i = 0; i < 4; ++i) {
        X[i] = get_word(input + i * 4);
    }

    for (int i = 0; i < 32; ++i) {
        word32 current_round_key = is_encrypt ? this->round_keys[i] : this->round_keys[31 - i];
        word32 temp = X[1] ^ X[2] ^ X[3] ^ current_round_key;
        X[0] = X[0] ^ T_lookup(temp);

        word32 next_X0 = X[1];
        X[1] = X[2];
        X[2] = X[3];
        X[3] = X[0];
        X[0] = next_X0;
    }

    for (int i = 0; i < 4; ++i) {
        put_word(X[3 - i], output + i * 4);
    }
}

void SM4_TTable_Context::encrypt(const word8 plaintext[16], word8 ciphertext[16]) {
    process_block(plaintext, ciphertext, true);
}

void SM4_TTable_Context::decrypt(const word8 ciphertext[16], word8 plaintext[16]) {
    process_block(ciphertext, plaintext, false);
}

int main() {
    const int NUM_TESTS = 1000;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, 255);

    std::vector<double> durations;
    durations.reserve(NUM_TESTS);

    word8 m[16], k[16], c[16], decrypted_m[16];
    SM4_TTable_Context ctx;

    for (int i = 0; i < NUM_TESTS; i++) {
        for (int j = 0; j < 16; j++) {
            m[j] = dis(gen);
            k[j] = dis(gen);
        }

        ctx.set_key(k);

        auto start = std::chrono::high_resolution_clock::now();
        ctx.encrypt(m, c);
        auto end = std::chrono::high_resolution_clock::now();

        std::chrono::duration<double, std::milli> duration_ms = end - start;
        durations.push_back(duration_ms.count());

        ctx.decrypt(c, decrypted_m);
        for (int j = 0; j < 16; ++j) {
            if (m[j] != decrypted_m[j]) {
                std::cerr << "Error: Decryption failed at test " << i << std::endl;
                return 1;
            }
        }
    }

    double total = std::accumulate(durations.begin(), durations.end(), 0.0);
    double average = total / NUM_TESTS;

    std::cout << "Correctness and Performance Test Completed." << std::endl;
    std::cout << "Total tests: " << NUM_TESTS << " random encryptions/decryptions." << std::endl;
    std::cout << "Average encryption time: " << std::fixed << std::setprecision(4) << average << " ms" << std::endl;
    std::cout << "Total time for all encryptions: " << std::fixed << std::setprecision(2) << total << " ms" << std::endl;

    return 0;
}